...
function cleanHash(value: unknown) { return String(value || "").trim(); }
function isHexHash(value: string) { return /^0x[a-fA-F0-9]{64}$/.test(value); }
function isBtcHash(value: string) { return /^[a-fA-F0-9]{64}$/.test(value); }

async function getUsdPrices() {
  const url = Deno.env.get("PRICE_API_URL") || "https://api.coingecko.com/api/v3/simple/price?ids=bitcoin,ethereum,tether&vs_currencies=usd";
  const response = await fetch(url, { headers: { accept: "application/json" } });
  if (!response.ok) throw new Error("Price provider unavailable");
  const data = await response.json();
  return { BTC: Number(data.bitcoin?.usd), ETH: Number(data.ethereum?.usd), USDT: Number(data.tether?.usd || 1) };
}
...
async function verifyBitcoin(txHash: string, expectedUsd: number) {
  ...
  if (!tx.status?.confirmed || !Number.isInteger(height)) return { ok: false, error: "Bitcoin transaction is not confirmed yet." };
  const tipResponse = await fetch(`${base}/blocks/tip/height`);
  if (!tipResponse.ok) return { ok: false, error: "Bitcoin confirmation status is temporarily unavailable." };
...
async function verifyEthereum(txHash: string, expectedUsd: number) {
  ...
  if (!receipt || receipt.status !== "0x1") return { ok: false, error: "Ethereum transaction has not successfully settled." };
  ...
  if (confirmations < MIN_CONFIRMATIONS) return { ok: false, confirmations, error: `Ethereum transaction has only ${confirmations} confirmation(s); ${MIN_CONFIRMATIONS} are required.` };
  if (String(tx.to || "").toLowerCase() !== COMPANY_WALLETS.ETH.toLowerCase()) return { ok: false, confirmations, error: "Ethereum recipient does not match the official settlement address." };
  const prices = await getUsdPrices();
  const receivedEth = Number(BigInt(tx.value || "0x0")) / 1e18;
  const expectedEth = expectedUsd / prices.ETH;
  if (receivedEth < expectedEth * (1 - USD_TOLERANCE)) return { ok: false, confirmations, error: "Ethereum transaction amount does not match the selected plan." };
  return { ok: true, confirmations, received: receivedEth };
}

async function verifyUsdtTrc20(txHash: string, expectedUsd: number) {
  if (!/^[a-fA-F0-9]{64}$/.test(txHash)) return { ok: false, error: "Invalid TRON transaction hash format." };
  ...
  if (!tx || tx.ret?.[0]?.contractRet !== "SUCCESS") return { ok: false, error: "TRON transaction has not successfully settled." };
  const block = Number(tx.blockNumber);
  const latestResponse = await fetch(`${apiUrl}/wallet/getnowblock`, { headers });
  if (!latestResponse.ok) return { ok: false, error: "TRON confirmation status is temporarily unavailable." };
  const latestBody = await latestResponse.json();
  const latest = Number(latestBody.block_header?.raw_data?.number);
  const confirmations = latest - block + 1;
  if (!Number.isFinite(block) || !Number.isFinite(latest) || confirmations < MIN_CONFIRMATIONS) return { ok: false, confirmations: Math.max(0, confirmations || 0), error: `TRON transaction has insufficient confirmations; ${MIN_CONFIRMATIONS} are required.` };
...
serve(async (req: Request) => {
  ...
  try {
    ...
    if (!plan || !(network in COMPANY_WALLETS) || !txHash || !payoutAddress) return json({ error: "Invalid investment verification request." }, 400);

    const { data: duplicate } = await supabaseAdmin.from("investments").select("id").ilike("tx_hash", txHash).maybeSingle();
    ...
    if (!result.ok) {
      console.warn("[verify-deposit] rejected transaction", { userId: user.id, planId, network, txHash, reason: result.error });
      return json({ error: result.error || "Transaction verification failed.", confirmations: result.confirmations || 0 }, 400);
    }
    ...
    const { data, error } = await supabaseAdmin.from("investments").insert(investment).select("id, plan_id, plan_name, amount, status, start_date, maturity_date, tx_confirmations").single();
    if (error) {
      if (error.code === "23505") return json({ error: "This transaction ID has already been used." }, 409);
      throw error;
    }
