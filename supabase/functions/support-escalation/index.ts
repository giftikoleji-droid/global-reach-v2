...
serve(async (req: Request) => {
  ...
  try {
    ...
    const querySummary = String(payload.query_summary || "Institutional support inquiry")
      .trim()
      .slice(0, 500);
    ...
    if (!supabaseUrl || !supabaseKey)
      throw new Error("Support service is not configured securely.");
    ...
    if (dbError) {
      console.error("[support-escalation] ticket insert failed", {
        code: dbError.code,
    ...
    const companyInbox = (Deno.env.get("COMPANY_INBOX") || "aetheriscapital.support@gmail.com")
      .trim()
      .toLowerCase();
    ...
    if (!resendApiKey) {
      console.error(
        "[support-escalation] RESEND_API_KEY is missing; ticket created but email was not attempted",
        { ticketId, companyInbox },
      );
      return json({
        success: true,
        ticket_id: ticketId,
        ...
        warning: "Ticket created, but email delivery is not configured.",
      });
    }

    if (!configuredFrom) {
      console.error(
        "[support-escalation] SUPPORT_FROM_EMAIL is missing; configure a verified Resend sender",
        { ticketId, companyInbox },
      );
      return json({
        success: true,
        ...
        status: "open",
        email_dispatched: false,
        warning: "Ticket created, but the support sender email is not configured.",
      });
    }
    ...
    console.log("[support-escalation] sending ticket email", {
      ticketId,
      companyInbox,
      from: configuredFrom,
    });
    ...
    if (!resendResponse.ok) {
      console.error("[support-escalation] Resend rejected email", {
        ticketId,
      ...
      return json({
        ...
        status: "open",
        email_dispatched: false,
        warning: `Ticket created, but email delivery failed (${resendResponse.status}).`,
      });
    }

    let resendResult: ResendResult | null = null;
    try {
      resendResult = JSON.parse(resendBody) as ResendResult;
    } catch {
      /* keep successful response even if non-JSON */
    }
    console.log("[support-escalation] email accepted by Resend", {
      ticketId,
