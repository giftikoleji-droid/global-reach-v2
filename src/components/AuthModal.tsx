...
export function AuthModal({
  ...
  function buildProfile(userId: string, userEmail: string, metadata?: Record<string, unknown>): Profile {
    ...
    return {
      ...
      ref_code: (typeof metadata?.ref_code === "string" && metadata.ref_code) || generateRefCode(normalized),
  ...
  async function handleContinue(e: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (!loginError && loginData.user) {
        const profile = await finishSupabaseUser(
          loginData.user.id,
          normalizedEmail,
      ...
      if (refCode) {
        signupMeta.referred_by = refCode;
      }
      ...
      if (signupError) {
        ...
        if (
          ...
          setMsg({
            text: "This email is already registered, but the password was not accepted. Please check your password or use the secure login link / reset option.",
            type: "error",
          });
        } else {
      ...
      if (signupData.user && !signupData.session) {
        setBusy(false);
        setMsg({
          text: "Account created. Please check your email to confirm your account, then return here to continue.",
          type: "info",
        });
        return;
      }
      ...
      if (signupData.user) {
        const profile = await finishSupabaseUser(
          signupData.user.id,
          normalizedEmail,
          signupData.user.user_metadata as Record<string, unknown> | undefined,
        );
      ...
      const message = err instanceof Error ? err.message : "We could not complete authentication. Please check your connection and try again.";
      ...
      >
        ...
        <div style={{ textAlign: "center", marginBottom: 22 }}>
          ...
          <p style={{ margin: "7px 0 0", color: "#94A3B8", fontSize: 13 }}>
            Enter your email and password to continue. We'll automatically sign you in or create
            your account.
          </p>
        </div>
        ...
        {linkSent ? (
          <div style={{ textAlign: "center", padding: "12px 0 4px" }}>
            ...
            <p style={{ color: "#E5E7EB", fontSize: 13, lineHeight: 1.5 }}>
          ...
          <>
            ...
            >
              <span style={{ height: 1, flex: 1, background: "#374151" }} />
              OR
              <span style={{ height: 1, flex: 1, background: "#374151" }} />
            </div>
