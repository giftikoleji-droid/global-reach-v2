...
export function DashboardHome({
  ...
  const isMatured =
    activeInvestment?.status === "matured" ||
    ...
    <div className="dashboard-page">
      <header className="dashboard-page-heading">
        ...
        <div className="dashboard-greeting">
          Welcome back, <strong>{profile?.name || "Client"}</strong>
        </div>
      </header>
      ...
      <section className="portfolio-hero-card">
        <div className="portfolio-hero-top">
          <div>
            ...
            <div className="portfolio-subvalue">
              {activeInvestment ? "Active mandate + available balance" : "No active mandate"}
            </div>
          </div>
          <div className="portfolio-change">
            <span>PORTFOLIO CHANGE</span>
            <strong className={metrics.changePct >= 0 ? "positive" : "negative"}>
              {metrics.changePct >= 0 ? "+" : ""}
              {metrics.changePct.toFixed(2)}% <ArrowUpRight size={20} />
            </strong>
        ...
        <div className="portfolio-stat-row">
          ...
          <MiniStat
            title="TOTAL PROFIT"
            value={`${metrics.profit >= 0 ? "+" : "-"}${money(Math.abs(metrics.profit))}`}
      ...
      {activeInvestment ? (
        <section className="active-mandate-card">
          <div className="section-heading-row">
            ...
            <span
              ...
              style={{
                color: isMatured ? "#10B981" : "#D4AF37",
                borderColor: isMatured ? "rgba(16,185,129,.35)" : "rgba(212,175,55,.35)",
                background: isMatured ? "rgba(16,185,129,.10)" : "rgba(212,175,55,.10)",
                whiteSpace: "nowrap",
              }}
            >
          ...
          <div className="mandate-overview">
            ...
            <div className="mandate-name">
              <span>Current mandate</span>
              <strong>{activeInvestment.plan_name}</strong>
              <small>
                {activeInvestment.return_pct}% target · {activeInvestment.term_days} days
              </small>
            </div>
            <div className="mandate-value">
              <span>Current Value</span>
              <strong className={metrics.current > metrics.invested ? "positive" : ""}>
                {money(metrics.current)}
              </strong>
            </div>
          </div>
          ...
          <div className="mandate-metrics">
            ...
            <MetricBlock
              title="Current Value"
              value={money(metrics.current)}
              positive={metrics.current > metrics.invested}
            />
          </div>
            ...
            <div className="progress-track">
              <div
                ...
                style={{
                  ...
                  background: isMatured ? "#10B981" : "#D4AF37",
                }}
              />
            </div>
            <div className="progress-footer">
              <span>
                {progress.remaining ? `${progress.remaining} days remaining` : "Maturity reached"}
              </span>
              <span>
                {activeInvestment.end_date || activeInvestment.maturity_date
                  ? `Matures ${formatDate(activeInvestment.end_date || activeInvestment.maturity_date!)}`
        ...
        <section className="empty-mandate-card">
          ...
          <button type="button" className="dashboard-button primary" onClick={onExplore}>
      ...
      <section className="activity-card">
        <div className="section-heading-row">
          <div>
            <div className="dashboard-eyebrow">Account Ledger</div>
            <h2>Recent Activity</h2>
          </div>
          <button type="button" className="text-action" onClick={onViewInvestments}>
            View Investments <ChevronRight size={16} />
          </button>
        </div>
        {transactions.length ? (
...
function ActivityRow({ transaction }: { transaction: Transaction }) {
  ...
  return (
    <div className="activity-row">
      <div className={`activity-icon ${isCredit ? "credit" : "debit"}`}>
